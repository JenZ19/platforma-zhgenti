...
export function ExpectedScene({ project, step }: { project: ProjectDefinition; step: number }) {
  ...
  return <main id="capture-scene" className="capture-canvas"><header><div className="capture-brand"><span>S</span> SUBMARINE</div><div>УРОВЕНЬ {String(step).padStart(2, "0")} · НЕДЕЛЯ {project.week}</div></header><section className="capture-title"><p>Вот что должно получиться</p><h1>{sourceTitle ?? titles[step - 1]}</h1><span>{project.title}</span></section><div className="capture-visual"><Visual project={project} step={step} /></div><aside className="capture-tip"><b>✦</b><p><strong>Сверь свой экран с примером.</strong><br />Мелкие отличия в тексте и цвете — это нормально.</p></aside></main>;
}
