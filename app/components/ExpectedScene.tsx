...
function BotScene({ project, step }: { project: ProjectDefinition; step: number }) {
  return <div className="phone-stage"><div className="phone-mock"><header><span>{project.symbol}</span><div><b>{project.title}</b><small>бот · онлайн</small></div></header><main><p className="bot-message">Здравствуйте! Я помогу: {project.outcome.toLocaleLowerCase("ru")}.</p><p className="user-message">{project.demo[0]}</p><p className="bot-message">Готово. Что сделать дальше?</p>{step >= 8 && <p className="user-message">{project.demo[1]}</p>}{step >= 9 && <p className="bot-message">Сохранила ответ и подготовила следующий шаг ✓</p>}<div className="bot-buttons">{project.features.slice(0, step >= 11 ? 4 : 2).map((feature) => <button key={feature}>{feature}</button>)}</div></main><footer>Сообщение… <b>↑</b></footer></div><div className="chat-check"><span>Тестовый диалог</span><h3>{step >= 11 ? "Весь путь пройден" : "Сценарий работает"}</h3><p>Ответы и кнопки используют только вымышленные данные.</p></div></div>;
}
