...
export function MobileAcademy({ onOpen }: { onOpen: (slug: string) => void }) {
  ...
  const [stats, setStats] = useState({ startedProjects: 0, completedProjects: 0, completedSteps: 0, totalProjects: 52, totalSteps: 884, score: 0 });

  useEffect(() => {
    // Mobile progress has its own namespace and is restored after mount.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setStats(getAcademyStats(projects.map((project) => ({ ...project, slug: `mobile:${project.slug}` })), window.localStorage));
  }, []);

  const visible = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase("ru");
    return projects.filter((project) => (week === 0 || project.week === week) && (!needle || `${project.title} ${project.outcome} ${project.track}`.toLocaleLowerCase("ru").includes(needle)));
  }, [query, week]);
  ...
  return (
    <main className="mobile-academy-shell">
      ...
      <section className="mobile-academy-hero">
        ...
        <div className="mobile-stats"><div><b>52</b><span>проекта</span></div><div><b>{stats.startedProjects}</b><span>начато</span></div><div><b>{stats.completedProjects}</b><span>готово</span></div></div>
      </section>
      ...
      <footer className="mobile-footer"><span>SUBMARINE</span><h2>Один телефон.<br />Пятьдесят два проекта.<br /><em>Новая профессия.</em></h2></footer>
    </main>
  );
}
