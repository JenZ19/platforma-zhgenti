...
function AcademyGuide({ project, frame, step }: { project: ProjectDefinition; frame: QuestGuideFrame; step: number }) {
  return <div className="original-guide-app original-guide-academy"><header><span>Н</span><b>НЕЙРОПРОФИ</b><small>{project.title}</small></header><aside><b>КАРТА КВЕСТА</b>{[step - 1, step, step + 1].filter((item) => item > 0 && item < 18).map((item) => <div key={item} className={item === step ? "active" : ""}><i>{String(item).padStart(2, "0")}</i><span>{item === step ? frame.title : `Соседний уровень ${item}`}</span></div>)}</aside><main><small>УРОВЕНЬ {String(step).padStart(2, "0")} · ТЕКСТ УЖЕ ГОТОВ</small><h3>{frame.title}</h3><p>{frame.action}</p>{step === 2 && project.journey !== "setup" ? <PaletteGuide /> : frame.exactText && <div className="original-guide-command"><span>ГОТОВАЯ КОМАНДА</span><p>{frame.exactText.slice(0, 370)}…</p><button>Скопировать команду</button></div>}<Pointer frame={frame}/></main></div>;
}
...
function CodexGuide({ project, frame, step }: { project: ProjectDefinition; frame: QuestGuideFrame; step: number }) {
  ...
  const beforeFiles = step === 3;
  return <div className="original-guide-app original-guide-codex"><aside><b>⌘ Codex</b><button>＋ Новая задача</button>{beforeFiles ? <><small>СЕЙЧАС</small><strong>Новый проект — пока без файлов</strong><span className="active">◉ Короткий разговор</span><span>1 вопрос за раз</span><span>Ответ голосом или текстом</span></> : <><button className="open">▱ Открыть проект</button><small>ПРОЕКТ</small><strong>⌄ {project.slug}</strong><span>✓ Паспорт подтверждён</span><span>✓ Рабочая основа</span><span className="active">◉ Уровень {String(step).padStart(2, "0")}</span></>}</aside><main><header><b>{beforeFiles ? "Новый проект" : project.slug}</b><span>{beforeFiles ? "Codex ждёт ваш первый ответ" : "Правильный проект открыт ✓"}</span></header><section>{showResult ? <div className="original-guide-answer"><small>✦ CODEX ЗАКОНЧИЛ</small><h3>{frame.doneWhen}</h3><p>Проверено внутри проекта «{project.title}».</p><div><i>✓</i><span>Уровень {step} готов к сверке</span></div></div> : <div className="original-guide-message"><small>ВАША КОМАНДА</small><p>{frame.exactText?.slice(0, 480) || frame.action}</p></div>}</section><footer><span>{frame.exactText ? "Команда вставлена — проверьте текст перед отправкой" : "Напишите задачу для Codex…"}</span><button>↑</button></footer><Pointer frame={frame}/></main></div>;
}

function PreviewGuide({ project, frame, step }: { project: ProjectDefinition; frame: QuestGuideFrame; step: number }) {
  return <div className="original-guide-preview"><div className="original-guide-browser"><span>● ● ●</span><b>{project.title} · предпросмотр</b><small>безопасная учебная схема</small></div><div className="original-guide-prototype">{project.journey === "setup" ? <SetupQuestPrototypeScene project={project} step={step}/> : isSourcePrototypeSlug(project.slug) ? <SourceProjectPrototypeScene project={project} step={step}/> : <OriginalServiceScene slug={project.slug} step={step}/>}</div><Pointer frame={frame}/></div>;
}
