...
export function MobileExpectedScene({ project, step }: { project: ProjectDefinition; step: number }) {
  ...
  if (project.slug === "family-expenses") {
    const titles = ["Готовый бюджет на телефоне","Моя версия сохранена","Расходы проверены","Серверная комната создана","Паспорт отправлен Фее","Основа готова","Первый расход сохранён","Свои категории добавлены","Свой стиль применён","Особенная функция работает","Семейный путь пройден","Изменение и копия проверены","Проверка одной рукой пройдена","Личная версия опубликована","Клиентская копия создана","Бриф заказчика применён","Две версии в портфолио"];
    return <main id="capture-scene" className="capture-canvas mobile-capture original-mobile-capture"><header><div className="capture-brand"><span>S</span> SUBMARINE</div><div>МОБИЛЬНЫЙ КВЕСТ · УРОВЕНЬ {String(step).padStart(2,"0")}</div></header><section className="capture-title"><p>Вот что должно получиться на телефоне</p><h1>{titles[step-1]}</h1><span>{project.title}</span></section><div className="mobile-capture-body"><div className="original-mobile-phone"><OriginalServiceScene slug={project.slug} step={step} mobile /></div><aside><div className={`mobile-capability ${capability.id}`}>{capability.label}</div><span>ШАГ {String(step).padStart(2,"0")}</span><h2>{titles[step-1]}</h2><p>Этот экран относится только к семейному бюджету. Цвет и название могут быть вашими.</p><div><b>✓</b> Одно действие с телефона</div><div><b>✓</b> Клиентская копия хранится отдельно</div></aside></div></main>;
  }
